    ...
    if (lastJsonMessage !== null) {
      if (lastJsonMessage.status === "noprofile") {
        ...
        const newItems = lastJsonMessage.result.filter(
          (item) => !result.find((o) => o.key === item.key)
        );
        if (newItems.length > 0) setResult((prev) => prev.concat(newItems));
      }
    }
  }, [readyState, initialLoad, result, sendJsonMessage, lastJsonMessage, pKey, coreId]);

  console.log("result: 53", result);
  return (
    <AppContext.Provider value={{ result, setResult, url, pKey, coreId }}>
      {result.length > 0 ? (
        <MagazinePage />
      ) : (
        <div className="root">
